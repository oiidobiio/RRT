        ...
        if x_create
            ...
            if contactFlag %若其中一个返回值不为0 则发生碰撞，结束，重新循环 
                if(S_newcost < S.totalcost(S_nearIndexlist(rewire_index)))
                    ...
                    l_draw_newlist(S_nearIndexlist(rewire_index)) = plot([S.x(S_nearIndexlist(rewire_index)), newCoor(1)], [S.y(S_nearIndexlist(rewire_index)), newCoor(2)],'b','Linewidth',0.5);
                end
                continue;
            end 
            ...
            l_draw_newlist(S_nearIndexlist(rewire_index)) = plot([S.x(S_nearIndexlist(rewire_index)), x_create(1)], [S.y(S_nearIndexlist(rewire_index)), x_create(2)],'b','Linewidth',0.5);
            end
        ...
        if x_create_E
            ...
            if contactFlag %若其中一个返回值不为0 则发生碰撞，结束，重新循环 
                if(E_newcost < E.totalcost(E_nearIndexlist(rewire_index)))
                    ...
                    l_draw_newlist(E_nearIndexlist(rewire_index)) = plot([E.x(E_nearIndexlist(rewire_index)), newCoor(1)], [E.y(E_nearIndexlist(rewire_index)), newCoor(2)],'b','Linewidth',0.5);
                end
                continue;
            end 
            ...
            l_draw_newlist(E_nearIndexlist(rewire_index)) = plot([E.x(E_nearIndexlist(rewire_index)), x_create_E(1)], [E.y(E_nearIndexlist(rewire_index)), x_create_E(2)],'b','Linewidth',0.5);
            end
